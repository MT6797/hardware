...
static int fingerprint_set_active_group(struct fingerprint_device __unused *dev,
	...
	int ret;
	ma_set_active_group(gid, store_path);	
    return ret<0? FINGERPRINT_ERROR: 0;
} 
...
static int fingerprint_open(const hw_module_t* module, const char __unused *id,
    ...
    dev->notify = NULL;
#ifdef YUD_ACQUIRED 
    //dev->setNavMode = fingerprint_setNavMode;
    //dev->getMode = fingerprint_getMode;
    //dev->setTapTimeout = fingerprint_setTapTimeout;
   // dev->setDoubleTapMinTime = fingerprint_setDoubleTapMinTime;
    //dev->setDoubleTapTimeout = fingerprint_setDoubleTapTimeout;
    //dev->setLongPressTimeout = fingerprint_setLongPressTimeout;
#endif
